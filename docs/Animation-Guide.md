...
| G | Hero flow field | `assets/js/hero-field.js` — three.js GPGPU (positions/velocities in textures via GPUComputationRenderer), simplex-noise flow field, pointer attraction, click pulse rings, UnrealBloom threshold 0.62 = selective glow | Homepage only (importmap in `default.html`). 16k particles on phones, 50k desktop, DPR ≤1.5, paused off-screen/hidden. Fallback = the CSS gradient; module exits on reduced-motion/no-WebGL2/any error. Audio hook: `window.__heroRingPulse(0..1)`. |
